...
function dataHash() {
  return JSON.stringify({
    ...
    s: window.pengaturan                     || {},
    k: (window.riwayatStok || []).slice(0, 20), // Bug Fix 7
  });
}
...
window.fbLoadAllData = async function() {
  ...
  try {
    ...
    if (snap.exists()) {
      ...
      // Bug Fix 8: load riwayat_stok dari cloud
      if (data.riwayat_stok) {
        window.riwayatStok = data.riwayat_stok;
        localStorage.setItem('riwayat_stok', JSON.stringify(data.riwayat_stok));
      }
...
window.fbListenRealtime = function() {
  ...
  onValue(dataRef, (snap) => {
    ...
    // Bug Fix 9: sync riwayat_stok dari realtime listener
    if (data.riwayat_stok && JSON.stringify(data.riwayat_stok) !== JSON.stringify(window.riwayatStok)) {
      window.riwayatStok = data.riwayat_stok;
      localStorage.setItem('riwayat_stok', JSON.stringify(data.riwayat_stok));
      berubah = true;
    }
...
window.fbSimpanSemua = async function() {
  ...
  try {
    await set(tokoRef('data'), {
      ...
      settings:     window.pengaturan  || {},
      riwayat_stok: window.riwayatStok || [],  // Bug Fix 6
    });
